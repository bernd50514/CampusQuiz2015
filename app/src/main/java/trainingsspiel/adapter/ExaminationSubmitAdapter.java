...
public class ExaminationSubmitAdapter extends PagerAdapter implements OnClickListener {
    ...
    public ExaminationSubmitAdapter(ChallengeQuizActivity context, List<View> viewItems, List<AnSwerInfo> dataItems, String imgServerUrl) {
        newContext = context;
        this.viewItems = viewItems;
        this.dataItems = dataItems;
        this.imgServerUrl = imgServerUrl;
            ...
            holder.layoutA.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (dataItems.get(position).getCorrectAnswer().contains("1")) {
                        // Erste Antwort ist richtig, diese hat der Nutzer gewählt
                        mContext.setCurrentView(position + 1); // zu nächster Fragemaske wechseln
                        holder.ivA.setImageResource(R.drawable.ic_practice_test_right_new); // Grüne Hacken für richtige Antwort
                        holder.tvA.setTextColor(Color.parseColor("#61bc31"));// Text der richtigen Antwort wurde grün markiert
                        ...
                        if (colunm == -1) {
                            Toast.makeText(mContext, "Falsche Antwort", Toast.LENGTH_SHORT).show();
                        }
            ...
            holder.layoutD.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    if (dataItems.get(position).getCorrectAnswer().contains("4")) {
                        ...
                        mContext.setCurrentView(position + 1); // next questions
                        holder.ivD.setImageResource(R.drawable.ic_practice_test_right_new);
                        holder.tvD.setTextColor(Color.parseColor("#61bc31"));
                        isCorrect = ConstantUtil.isCorrect;
                        ...
                        if (colunm == -1) {
                            Toast.makeText(mContext, "Falsche Antwrot", Toast.LENGTH_SHORT).show();
                        }
            ...
            if (mPosition == viewItems.size()) {
                ...
                if (dataItems.get(mPosition1).getQuestionType().equals("0")) {
                    if (!map.containsKey(mPosition1)) {
                        Toast.makeText(mContext, "Hast du schon geantwortet?", Toast.LENGTH_SHORT).show();
                        return;
                    }
